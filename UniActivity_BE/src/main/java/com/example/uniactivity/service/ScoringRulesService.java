...
public class ScoringRulesService {
    ...
    /**
     * Get criteria name by code
     * Phase 4: Added for notification messages
     */
    public String getCriteriaName(String criteriaCode) {
        if (criteriaCode == null || criteriaCode.isEmpty()) {
        ...
        for (JsonNode category : categories) {
            ...
            for (JsonNode subcategory : subcategories) {
                if (subcategory.get("id").asText().equals(criteriaCode)) {
                    return subcategory.get("name").asText();
                }
            }
        }
        return criteriaCode; // Return code if name not found
    }
}
