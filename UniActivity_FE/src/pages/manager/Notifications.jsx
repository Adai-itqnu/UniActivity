...
export default function Notifications() {
    ...
    const fetchPage = useCallback((p = 0, append = false) => {
        ...
        fetch(`/manager/api/notifications?page=${p}&size=20`, { credentials: 'include' })
    ...
    const fetchUnread = useCallback(() => {
        fetch('/manager/api/notifications/unread-count', { credentials: 'include' })
    ...
    const handleMarkRead = async (id) => {
        await fetch(`/manager/api/notifications/${id}/read`, { method: 'POST', credentials: 'include' })
        setNotifications(prev => prev.map(n => n.id === id ? { ...n, isRead: true } : n))
        fetchUnread()
    }

    const handleMarkAllRead = async () => {
        await fetch('/manager/api/notifications/read-all', { method: 'POST', credentials: 'include' })
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
                {loading && page === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">Thông báo sẽ hiển thị khi có yêu cầu mới</p>
                    </div>
                )}
            </div>
        </div>
